...
### User with Account Signing Key
```terraform
# This example shows how to use account signing keys for enhanced security
# Signing keys enable key rotation without regenerating user JWTs
...
resource "nsc_nkey" "account_signing" {
  type = "account"  # Signing keys are also account-type keys
}
...
resource "nsc_account" "app" {
  ...
  subject      = nsc_nkey.account.public_key
  issuer_seed  = nsc_nkey.operator.seed
  signing_keys = [nsc_nkey.account_signing.public_key]  # Register signing key
}

# Create user signed with the signing key
resource "nsc_user" "service" {
  name           = "ServiceUser"
  subject        = nsc_nkey.user.public_key
  issuer_seed    = nsc_nkey.account_signing.seed    # Sign with signing key
  issuer_account = nsc_nkey.account.public_key      # Account's primary key
...
# 1. Key rotation: Rotate signing keys without changing the account identity
# 2. Separation of concerns: Store primary keys securely, use signing keys in CI/CD
# 3. Least privilege: Distribute signing keys to teams without exposing primary keys
# 4. Compromise recovery: Revoke compromised signing keys without regenerating everything
```
